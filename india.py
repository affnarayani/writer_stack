...
def run():
    ...
    try:
        context = pw.chromium.launch_persistent_context(
            user_data_dir="medium_user_data",
            headless=HEADLESS,
            no_viewport=True,
        ...
        user_button = page.get_by_role('button', name='Ujjawal Kumar')
        
        # Checking visibility with a short delay to handle potential rendering lags
        if user_button.is_visible(timeout=5000):
            print("Login Success", flush=True)
        else:
            print("Problem in Login", flush=True)
            sys.exit(1)
        ...
        caption_element = page.locator('div[contenteditable="true"][data-placeholder*="Type caption"]')
