...
def mark_article_as_posted(file_path: str):
    ...
    try:
        ...
        # Remove if key already exists somewhere to ensure it drops at the absolute bottom
        if "posted" in data:
            del data["posted"]
            
        # Append target key-value pair at the end
        data["posted"] = True
        
        # Write back indented clean JSON
...
def run():
    ...
    # Order wise dynamic paragraph keys nikalna (excluding metadata)
    ...
    try:
        ...
        for key in content_keys:
            ...
            # Focusing on paragraph locator for the first text injection
            if is_first_para:
            ...
            if key == "p_cta" and "http" in para_text:
                ...
                # Split content into raw sentence, anchor text, and URL
                parts = para_text.split("http")
                target_url = "http" + parts[1].strip()
                
                text_before_url = parts[0].strip()
                
                # Split at 'Click Here' to segregate body text from link text, removing trailing colon
                ...
                # 1. Type pure body message first
                ...
                # 2. Type exclusive anchor link text
                for char in display_text:
                    page.keyboard.type(char)
                    time.sleep(random.uniform(0.04, 0.12))
                custom_random_wait(2, 4)

                # 3. Precision Select: Shift+ArrowLeft
                ...
                # Normal paragraph validation block
                for char in para_text:
                    page.keyboard.type(char)
                    time.sleep(random.uniform(0.03, 0.10))
        ...
        # 1. Click primary Publish button
        ...
        # 2. Wait for 15-30 seconds random delay
        intermediate_publish_wait()

        # 3. Handle Keywords Tag Injection Phase
        if chosen_keywords:
        ...
        # 4. Execute final submit action button click
        ...
        # Final safety timeout before environment shutdown
        intermediate_publish_wait()
